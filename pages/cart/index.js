...
Page({
  ...
  setCart(cart) {
    // 1 计算全选
    //  every 数组方法，hi遍历、接收一个回调函数，那么，每一个回调函数都返回true，那么every的返回值为true
    //  只要有一个遍历项返回false，则不再循环，直接返回false
    //  空数组调用，返回值就是true
  ...
  async handleItemNumEdit(e) {
    ...
    if (cart[index].num === 1 && operation === -1) {
      // 4.1 弹窗提示
      const res = await showModal({ content: "是否要删除 "+cart[index].goods_name });
      if (res.confirm) {
        cart.splice(index, 1);
        this.setCart(cart);
      }
